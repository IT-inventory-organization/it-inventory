  ...
  try {
    ...
    if (total.length === 0) {
      return successResponse(
        res,
        httpStatus.ok,
        `Data With Status "${status}" Is Empty`
      );
    }
    ...
    return successResponse(
      ...
      httpStatus.ok,
      "",
      importExportTotal(total, status)
    );
  } catch (error) {
    return errorResponse(
      res,
      httpStatus.badRequest,
      "Failed To Get Total Report Type Data"
    );
  }
};
...
const convertDate = (date) => {
  ...
  return `${addZero(ISO.getDate())}-${addZero(
    ISO.getMonth() + 1
  )}-${ISO.getFullYear()}`;
};
...
const getXMLReport = async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < listBarang.length; i++) {
      ...
      xml += `<KD_KANTOR>${getCode(
        result.DataPengajuan.kantorPabeanAsal
      )}</KD_KANTOR>`;
      xml += `<KD_JNS_ANGKUT>${getCode(
        result.DataPengangkutan.caraAngkut
      )}</KD_JNS_ANGKUT>`;
      ...
      xml += `<KD_PEL_MUAT>${removeSpace(
        result.DataPelabuhanMuatBongkar.pelabuhanMuat
      )}</KD_PEL_MUAT>`;
      ...
      xml += `<JNS_ID_PENERIMA>${getCode(
        result.reportIdentitasPPJK.jenisIdentitasPPJK
      )}</JNS_ID_PENERIMA>`;
      ...
      xml += `<JNS_ID_PEMBERITAHU>${getCode(
        result.DataPengajuan.kategoryPemberitahuan
      )}</JNS_ID_PEMBERITAHU>`;
...
const getDataActivityRedLine = async (req, res) => {
  try {
    ...
    return errorResponse(
      res,
      httpStatus.internalServerError,
      "Failed To Fetch Red Line Activity"
    );
  }
};
...
const getDataHeaderReport = async (req, res) => {
  ...
  try {
    ...
    const dataPengajuanRes = await getPerTable(
      reportDataPengajuan,
      idReport,
      type,
      transaction
    );
    const identitasPenerimaRes = await getPerTable(
      reportIdentitasPenerima,
      idReport,
      type,
      transaction
    );
    const identitasPengirimRes = await getPerTable(
      reportIdentitasPengirim,
      idReport,
      type,
      transaction
    );
    const transaksiPerdaganganRes = await getPerTable(
      reportTransaksiPerdagangan,
      idReport,
      type,
      transaction
    );
    const dataPengakutanRes = await getPerTable(
      reportDataPengangkutan,
      idReport,
      type,
      transaction
    );
    const pelabuhanMuatBongkarRes = await getPerTable(
      reportDataPelabuhanMuatBongkar,
      idReport,
      type,
      transaction
    );
    const dataBeratDanVolRes = await getPerTable(
      reportDataBeratDanVolume,
      idReport,
      type,
      transaction
    );
    const dataPetiKemasDanPengemasRes = await getPerTable(
      reportDataPetiKemasDanPengemas,
      idReport,
      type,
      transaction
    );
    const dataTempatPnimbunanRes = await getPerTable(
      reportDataTempatPenimbunan,
      idReport,
      type,
      transaction
    );
    const datePerkiraanTanggalPengeluaranRes = await getPerTable(
      reportDataPerkiraanTanggalPengeluaran,
      idReport,
      type,
      transaction
    );
    const payload = {
      ...
      dataPerkiraanTanggalPengeluaran:
        datePerkiraanTanggalPengeluaranRes.toJSON(),
    };
...
const getDataLanjutanReport = async (req, res) => {
  ...
  try {
    ...
    const dataDokumenRes = await getPerTableBarangDokumen(
      reportListDokumen,
      idReport,
      type,
      transaction
    ...
    dataDokumenRes.forEach((res) => {
      dataDokumen.push(res.dataValues);
    });

    const dataPetiKemasRes = await getPerTable(
      reportDataPetiKemas,
      idReport,
      type,
      transaction
...
const getDataBarangReport = async (req, res) => {
  try {
    ...
    const dataBarangRes = await getPerTableBarangDokumen(
      reportListBarang,
      idReport,
      type
    );
...
const getDashboard = async (req, res) => {
  try {
    const pemasukan = await Report.count({
      where: { jenisPemberitahuan: "Pemasukan", isDelete: false },
    });
    const pengeluaran = await Report.count({
      where: { jenisPemberitahuan: "Pengeluaran", isDelete: false },
    });
...
module.exports = (routes) => {
  ...
  routes.get("/count", authentication, getDashboard);
};
